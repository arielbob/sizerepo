...
import { CLOTHING_GENDERS_SHORT_TO_FULL, UNITS } from '../../../common/constants'
...
const PostInfo: React.SFC<PostCardProps> = ({ data, units }) => {
  const {
    id,
    brand,
    article_name,
    ...
    <div className='leading-tight'>
      <Link to={'/posts/' + id}>
        ...
        <div className='flex items-center mb-2'>
          <h2 className='truncate inline-block mr-2 font-semibold'>{article_name}</h2>
      ...
      <div className='mb-2 text-sm font-mono'>
        ...
        </svg>
        {
          ...
          <>
            <span className=''>{metresToCmString(height_m)}</span>
            <span className='mx-1'>|</span>
            <span className=''>{kgsToKgsString(weight_kg)}</span>
          </>
        }
      </div>
      <div className='mb-1'>
